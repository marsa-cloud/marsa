...
- **With `--tls`, Traefik restarts _outside anything `install.sh` waits for_.** The chart ships `templates/cert-resolver.yaml`, a `HelmChartConfig` for Traefik in `kube-system`; helm-controller begins reconciling it as soon as it lands — which is _during_ the `helm --wait` phase, not after it — and re-runs Traefik's install job, so Traefik restarts with the `le` cert resolver absent from its config until that job completes. Note the shape precisely: this is **concurrent and unobserved**, not "after we finish". `install.sh` never waits for or observes it, and `helm --wait` does not watch Traefik, so the install does not fail — it is a window, not a breakage. That distinction is what explains a flaky _first_ TLS request; a clean-ordering reading would not. Invisible to CI because both E2E entry points pass `--no-tls`. Pre-existing behaviour, not introduced here; noted because this wait might otherwise be read as proving Traefik is settled for good.
