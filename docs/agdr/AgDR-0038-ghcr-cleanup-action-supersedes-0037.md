...
A grep confirms **nothing** in marsa currently consumes attestations (no cosign/scanner/admission policy), but they are retained deliberately for future supply-chain verification, so the fix must preserve *live* attestations, not disable them.
