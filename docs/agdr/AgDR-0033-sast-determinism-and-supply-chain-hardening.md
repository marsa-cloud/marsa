...
- PRs scan diff-aware against the base commit, so a future registry rule addition can no longer flip an unrelated PR red (the exact failure mode from #103). `main` still full-scans; if the registry later surfaces a finding on existing `main` code, that shows on the `main` run as a triage signal, not a PR blocker.
