...
const scaleFromAnchor = (hex) => {
  const [h, s] = rgbToHsl(...hexToRgb(hex))
  return scaleFromHS(h, s)
}
...
const swatchRow = (label, scale) => {
  const cells = STEPS.map(
    (s) =>
      `<div class="sw" style="background:${scale[s]};color:${textOn(scale[s])}"><b>${s}</b><span>${scale[s]}</span></div>`,
  ).join('')
  return `<div class="scale"><div class="name">${label}</div><div class="cells">${cells}</div></div>`
}
const brandCells = Object.entries(palette.brand)
  .map(
    ([k, hex]) =>
      `<div class="sw big" style="background:${hex};color:${textOn(hex)}"><b>${k}</b><span>${hex}</span></div>`,
  )
  .join('')
