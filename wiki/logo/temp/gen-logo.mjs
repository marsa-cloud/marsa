...
const P = {
  ...
  d_cp_lb: 118, d_cp_rb: 134, d_rb_eb: 125,
};
...
function connector(a, Ra, b, Rb, deg, sgn, id) {
  ...
  const nx = Math.cos(deg * D), ny = Math.sin(deg * D);   // axis
  const px = -ny, py = nx;                                 // perpendicular
  const o = sgn * P.OFF, hw = P.W / 2;
  const corner = (pt, s) => ({ x: pt.x + px*(o + s*hw), y: pt.y + py*(o + s*hw) });
  const pts = [corner(start,+1), corner(end,+1), corner(end,-1), corner(start,-1)];
  return { pts, start:{x:start.x+px*o,y:start.y+py*o}, end:{x:end.x+px*o,y:end.y+py*o}, id };
}
...
const berthPath = (c, R, A) =>
  pathFromPts(hexPts(c, R)) + ' ' + pathFromPts(holePts(c, P.RI, A));
...
// bounds
const allX = [CP,LB,RB,EB].flatMap((c,i)=>{const R=[P.R_cp,P.R_berth,P.R_berth,P.R_empty][i];return[c.x-R,c.x+R];});
const allY = [CP,LB,RB,EB].flatMap((c,i)=>{const R=[P.R_cp,P.R_berth,P.R_berth,P.R_empty][i];return[c.y-R,c.y+R];});
const pad = 16;
const minX = Math.min(...allX)-pad, minY = Math.min(...allY)-pad;
const w = Math.max(...allX)-minX+pad, h = Math.max(...allY)-minY+pad;
