...
- **Placement**: `project/ ← environment/ ← app-management/ ← release/`. App-keyed repositories load an app's project and environment through `selectAppPlacement` (`app-management/queries/app-placement.ts`), and features pass that placement (or `{ project, environment }`) straight to the runtime — `AppRef` / `EnvironmentRef` are nested shapes the rows already fit. How that maps to a Kubernetes namespace is private to the runtime adapter (`src/modules/runtime/adapters/kubernetes/environment/namespace-name.ts`). `EnvironmentRuntime` provisions an environment on create and on every deploy, and removes it with the environment.
