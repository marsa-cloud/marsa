...
- Because the SWC build ignores the `@nestjs/swagger` CLI plugin, response shapes must come from **explicit `@ApiProperty` decorators on a DTO class** (an exported `interface` produces no schema). Annotate controllers with `@ApiOkResponse({ type: Dto })`.
- **Never hand-write `@ApiOperation({ operationId })`.** A global `operationIdFactory` (`src/modules/swagger/operation-id-factory.ts`, wired into `createOpenApiDocument`) derives every id from the controller class name + URI version — `GetApiInfoController` + `v1` → `getApiInfoV1`. The **class name is the contract**: renaming a controller renames its operationId and churns the generated web types, so treat it as a public-surface change. A hand-written `operationId` overrides the factory and reintroduces exactly the drift it removes. AgDR-0040.
- **The factory assumes one route method per controller.** It ignores the method key, so a second `@Get`/`@Post` on an existing controller silently emits a duplicate operationId (invalid OpenAPI; the web generator collides). Keep controllers single-endpoint — the CQRS slice shape already does — or fold the method key in before adding a second route.
- **`operationId` convention: version-suffix it** (e.g. `getApiInfoV1`, not `getApiInfo`) — now produced by the factory rather than by hand. The web's generator names the operation-response type after the operationId; without the version suffix it collides with the DTO schema type and emits an ugly `…Response2` alias. Suffixing keeps the schema type (`GetApiInfoResponse`) and the operation type (`GetApiInfoV1Response`) distinct, and mirrors the URI version.
...
The `operationId` is **derived automatically** by the global factory from the controller class name (`<Action>Controller` → `<action>V1`; see the "OpenAPI contract" section above) — so a use-case named `view-app-index` yields `viewAppIndexV1` with nothing hand-written. The response / command / repository classes take the `<Action>` prefix (`ViewAppIndexResponse`, `DeployAppCommand`). marsa#131 migrated the first batch: `list-apps → view-app-index`, `get-app-health → view-app-health`, `get-app-run-logs → view-app-logs`, `list-app-releases → view-release-index`, `get-current-user → view-me`.
