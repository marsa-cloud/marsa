...
} from '#src/app/database-management/use-cases/view-database-index/view-database-index.response.js'
...
export class ViewDatabaseIndexUseCase {
  ...
  async execute(query: ViewDatabaseIndexQuery): Promise<ViewDatabaseIndexResponse> {
    ...
    const statuses = await Promise.all(
      placements.map((placement) => this.runtime.readStatus(placement)),
    )
