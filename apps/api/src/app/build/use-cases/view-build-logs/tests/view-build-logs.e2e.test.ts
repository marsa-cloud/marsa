...
import { BUILD_LOGS_EXPIRED } from '#src/app/build/use-cases/view-build-logs/view-build-logs.use-case.js'
...
describe('GET /api/v1/apps/:slug/builds/:buildUuid/logs (e2e)', () => {
  ...
  before(async () => {
    ...
    expired = new BuildBuilder().withApp(app).build()
  ...
  it('404s once the logs are gone', async () => {
    ...
    expect(response.body.message).toBe(BUILD_LOGS_EXPIRED)
  })
