...
describe('GET /api/v1/deployments/apps/:slug/logs (e2e)', () => {
  let setup: TestSetup
  ...
  it('returns a run-log snapshot (mock backend)', async () => {
    const response = await request(setup.httpServer)
      .get(`/api/v1/deployments/apps/${SLUG}/logs`)
  ...
  it('rejects a tailLines above the maximum with 400', async () => {
    await request(setup.httpServer)
      .get(`/api/v1/deployments/apps/${SLUG}/logs?tailLines=5000`)
      .set('Cookie', sessionCookie)
      .expect(400)
  })

  it('rejects an unauthenticated request with 401', async () => {
    await request(setup.httpServer).get(`/api/v1/deployments/apps/${SLUG}/logs`).expect(401)
  })
})
